      ...
      >
        <div className="grid grid-cols-1 md:grid-cols-4 gap-12 mb-20">
          ...
          >
            ...
            <h4 className="text-white font-black uppercase text-xs tracking-[0.3em] mb-4 italic">Next Drop</h4>
            <p className="text-slate-500 text-xs mb-6 font-medium">Join 2,000+ drivers and get notified when a new asset hits the hangar.</p>
            <div className="space-y-3">
              <input 
                type="email" 
                placeholder="EMAIL ADDRESS" 
                className="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-3 text-xs font-bold text-white focus:border-cyan-500 outline-none transition-all"
              />
              <motion.button 
                whileHover={{ scale: 1.02 }}
                whileTap={{ scale: 0.98 }}
                className="w-full bg-white hover:bg-cyan-500 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest text-black transition-all shadow-xl shadow-white/5"
              >
                Stay Updated
              </motion.button>
            </div>
