...
import { Globe, Calendar, TrendingUp, Trophy, Construction } from "lucide-react";
        ...
        <section className="py-24 bg-[#020617] relative overflow-hidden">
            <div className="container mx-auto px-6">
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 mb-20">
                    <div className="relative group p-1 bg-linear-to-br from-yellow-500/50 to-transparent rounded-[3rem]">
                        <div className="bg-[#0f172a] p-10 rounded-[2.9rem] h-full border border-slate-800">
                            ...
                            <p className="text-slate-400 text-sm leading-relaxed mb-8 italic">The ultimate battle for glory. Top drivers from all major bus simulation communities compete once a year.</p>
                ...
                <div className="text-center mt-20">
                    ...
                    {loading ? (
                         <div className="text-slate-500 animate-pulse font-black italic uppercase tracking-widest text-sm">Scanning Hangar...</div>
                    ) : topThree.length > 0 ? (
                        <div className="flex flex-col md:flex-row items-center md:items-end justify-center gap-10 md:gap-4 lg:gap-8">
                            {topThree.map((player) => (
                                <motion.div
                                    ...
                                    viewport={{ once: true }}
                                    className={`relative w-full max-w-72 md:w-64 lg:w-72 group ${
                                        player.rank === 1 ? 'order-1 md:order-2 z-10' :
                                        player.rank === 2 ? 'order-2 md:order-1' : 'order-3'
                                    }`}
                                >
                                    <div className={`absolute -inset-1 blur-2xl rounded-full transition-all duration-500 opacity-20 group-hover:opacity-40 ${player.rank === 1 ? 'bg-cyan-500' : 'bg-slate-500'}`}></div>

                                    <div className={`relative bg-slate-900/90 backdrop-blur-xl border-2 p-6 rounded-[2.5rem] transition-all duration-300 ${
                                        player.rank === 1 ? 'border-cyan-500 shadow-[0_0_20px_rgba(6,182,212,0.3)] md:pb-12' : 'border-slate-800'
                                    }`}>
                                        <div className={`absolute -top-4 left-1/2 -translate-x-1/2 px-6 py-1 rounded-full text-[10px] font-black uppercase tracking-widest border shadow-lg ${
                                            player.rank === 1 ? 'bg-cyan-600 border-cyan-300 text-black' : 'bg-slate-800 border-slate-600 text-slate-300'
                                        }`}>
                                            Rank #{player.rank}
                                        </div>

                                        <div className="flex flex-col items-center">
                                            <div className={`relative p-1 rounded-full mb-6 shadow-lg ${
                                                player.rank === 1 ? 'bg-linear-to-tr from-cyan-400 to-blue-500 shadow-cyan-500/50' : 'bg-slate-700 shadow-black'
                                            }`}>
                                                <img
                                                    src={player.photoURL || "https://i.ibb.co/Placeholder.png"}
                                                    alt={player.name}
                                                    className="w-24 h-24 md:w-28 md:h-28 rounded-full object-cover border-4 border-slate-900"
                                                />
                                            </div>

                                            <h3 className="text-white font-black uppercase text-lg tracking-tighter italic truncate w-full text-center">
                                                {player.name}
                                            </h3>

                                            <div className="mt-4 w-full bg-slate-950/60 rounded-2xl p-4 border border-slate-800">
                                                <p className="text-slate-500 text-[9px] font-black uppercase tracking-[0.2em] mb-1 text-center">Monthly Pts</p>
                                                <div className="text-cyan-400 font-black text-2xl flex items-center justify-center gap-2">
                                                    ...
                                                    {player.monthlyPoints?.toLocaleString() || 0}
                                                </div>
                                            </div>
                                        </div>
                        ...
                        <motion.div 
                            initial={{ opacity: 0 }}
                            animate={{ opacity: 1 }}
                            className="bg-slate-900/50 border border-slate-800 border-dashed p-12 rounded-[3rem] max-w-2xl mx-auto"
                        >
                            <Construction className="text-cyan-500 w-12 h-12 mx-auto mb-4 animate-pulse" />
                            <h3 className="text-xl font-black text-white italic uppercase tracking-tighter">Season Initializing...</h3>
                            <p className="text-slate-500 text-sm mt-2 italic leading-relaxed">
                                Current month competition has not officially started yet. <br />
                                Check back soon once the terminals begin updating driver points!
                            </p>
                        </motion.div>
                    )}
                </div>
